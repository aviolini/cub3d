...
/*   Updated: 2021/04/09 16:17:18 by aviolini         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	image->img = mlx_new_image(win->mlx, win->settings.mapW * \
		SCALE, win->settings.mapH * SCALE);
...
void	column(t_window *win, t_image *img, int x, int orientation)
{
	...
	while ((i) < wallbottomy - 1)
	{
		...
		(int)((win->ray.indexTex-(int)win->ray.indexTex)*win->texture[orientation].texW)));
			...
			//if (color >= 4278190080)   //SE E TRASPARENTE
			//	dst = 255;
		//else
				*(unsigned int*)dst = color;
	}
